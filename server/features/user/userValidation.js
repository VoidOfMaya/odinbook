...
const userEdit = [
    body('name').optional({ values: 'falsy' }).trim().isLength({max:25, min: 3}).withMessage('name must be 3 - 25 characters in length')
    .matches(/^[a-zA-Z ]+$/).withMessage('name can have letters and spaces'),
    body('bio').optional({ values: 'falsy' }).trim().isLength({max:150}).withMessage('bio limit is 250 characters in length')
